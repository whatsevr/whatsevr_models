...
void main() {
  group('WalletEarnings.applicationStatus', () {
    ...
    test('maps every status the server can send', () {
      ...
      expect(parse('approved').applicationStatus, HostApplicationStatus.approved);
      expect(parse('rejected').applicationStatus, HostApplicationStatus.rejected);
      expect(
    ...
    test('never having applied is not the same as being rejected', () {
      ...
      expect(parse('rejected').applicationStatus, isNot(parse(null).applicationStatus));
    });

    test('an unknown status degrades to none, not to a blank screen', () {
      expect(parse('something_new').applicationStatus, HostApplicationStatus.none);
    });
